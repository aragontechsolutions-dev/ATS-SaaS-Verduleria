...
export function corteHtml(c: Corte): string {
  ...
  const filasMedio = medios.length
    ? medios.map((m) => `<div class="row"><span>${MEDIO_LABEL[m] ?? m}</span><span>${money.format(c.porMedio[m])}</span></div>`).join('')
...
<style>
  * { font-family: 'Courier New', monospace; }
  ...
  <div class="sec">
    <div class="row"><span>Fondo de apertura</span><span>${money.format(c.montoApertura)}</span></div>
  ...
  <div class="sec">
    <div class="row"><span><b>Efectivo esperado</b></span><span><b>${money.format(c.efectivoEsperado)}</b></span></div>
    ${c.tipo === 'Z' && c.montoCierre != null ? `
      <div class="row"><span>Efectivo contado</span><span>${money.format(c.montoCierre)}</span></div>
      <div class="big"><span>Diferencia</span><span>${dif > 0 ? '+' : ''}${money.format(dif)}</span></div>` : ''}
  </div>
  <div class="foot">${c.tipo === 'Z' ? 'Turno cerrado' : 'Corte parcial — la caja sigue abierta'}</div>
</body></html>`;
}

/** Abre el corte en una ventana e imprime. */
export function printCorte(c: Corte): void {
  const w = window.open('', 'ATS_Corte', 'width=320,height=600');
  if (!w) return;
